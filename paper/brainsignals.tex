...
\end{align}
где функция $\Sim$ отражает похожесть между парой признаков, а функция $\Rel$ --- релевантность признака целевому вектору.
...
\begin{align}
\label{eq::brel_compute} \brel = [\brelel_{i}] = \Rel(\xfeat_i).
\end{align}

Функции $\Sim$, $\Rel$ выбираются в соответствии с решаемой задачей
