...
Задана выборка $\mathfrak{D}= \left( \bX, \bY \right)$, где $\bX \in \RR^{m \times n}$ --- матрица объектов, $\bX = \left[ \bchi_1 \dots \bchi_n \right]$, где $\bchi_j \in \mathbb{R}^{m}$ --- вектор значений $j$-го признака на элементах выборки; $\bY \in \RR^{m \times r}$ --- матрица ответов. Имеется линейная модель $\bff$ с набором параметров $\btheta$ из пространства $\RR^{n \times r}$, предсказывающая
...
\begin{equation}
\btheta^* = \arg \min_{\btheta  \in \RR^{|\cA| \times r}}   S(\theta, \cA | \mathfrak{D}, \bff)	\label{eq::fs_problem}
\end{equation}


Обозначим $X_{\cA}$ матрицу, столбцы которой --- векторы признаков, индексы которых находятся в $\cA$. В случае $X_{\cA}$, 
